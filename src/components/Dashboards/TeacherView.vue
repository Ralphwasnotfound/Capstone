<template>
    <div class="flex flex-row  gap-5">
        <!-- SIDE BAR -->
        <div class="bg-[#073050] flex flex-col items-start p-4 w-[25%] h-screen">
            ...
            <div class="w-full overflow-y-auto flex flex-col gap-2 pr-2 scrollbar-thin" style="max-height: calc(100vh - 140px)">
                ...
                @click="teacherSelectSection('dashboard')" 
                to="/dashboard/teacher"/>
                <ButtonSubjectsCourses
                :isActive="activeSection === 'subject'"
                @click="teacherSelectSection('subject')" 
                to="/dashboard/teacher/subject-courses"/>
                <ButtonAttendance
                :isActive="activeSection === 'attendance'"
                @click="teacherSelectSection('attendance')" 
                to="/dashboard/teacher/attendance"/>
                <ButtonDigitalID
                :isActive="activeSection === 'digital'"
                @click="teacherSelectSection('digital')" 
                to="/dashboard/teacher/digital-IDs"/>
                <ButtonGrades 
                :isActive="activeSection === 'grades'"
                @click="teacherSelectSection('grades')" 
                to="/dashboard/teacher/grades"/>
                <ButtonAnouncements
                :isActive="activeSection === 'announcements'"
                @click="teacherSelectSection('announcements')" 
                to="/dashboard/teacher/announcements"/>
                <ButtonConcernContact 
                :isActive="activeSection === 'contact'"
                @click="teacherSelectSection('contact')" 
                to="/dashboard/teacher/contact-concern"/>
                <ButtonProfileSettings 
                :isActive="activeSection === 'profile'"
                @click="teacherSelectSection('profile')" 
                to="/dashboard/teacher/profile-settings"/>
                <ButtonReports 
                :isActive="activeSection === 'reports'"
                @click="teacherSelectSection('reports')" 
                to="/dashboard/teacher/reports"/>
            </div>
        </div>
        
        <div class="flex-1 p-6 overflow-y-auto h-screen">
            <component :is="currentComputed" :role="'teacher'" :key="componentKey" />
        </div>
    </div>
</template>
